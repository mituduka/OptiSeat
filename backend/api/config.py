...
# ここは求解品質を優先した小さい値を既定とする。ただし小さすぎる（例: 0.05）と
# 1ショット目の探索降下がほぼ決定的になり、実行をまたいで第1候補が毎回同じ
# 配置になる。0.1 は品質劣化なしで実行間の多様性を保てる実測値。
# clingo が受け付けるのは 0〜1 のためこの範囲にクランプする。
SOLVER_RAND_FREQ: float = min(
    max(_env_float("SOLVER_RAND_FREQ", 0.1), 0.0), 1.0
)
