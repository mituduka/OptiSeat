    ...
    try:
        ctrl = Control(
            arguments=[
                ...
                #
                # 【optN + -n 1 でなければならない理由（実測済み・変更禁止）】
                # 弱制約が1つも grounding されないケース（配慮タグ・トグル・前回席が
                # すべて無い場合）が存在するため、両ケースを正しく扱える組は optN+-n 1 のみ:
                #   - opt + -n 1: 最適化時に「最初の改善解」で止まり最悪解を返す
                #   - opt + -n 0: 弱制約なしのとき全モデル列挙になりショット時間を使い切る
                #   - optN + -n 1: 最適化時は最適解1件で終了、弱制約なしでも1モデルで終了
                "-n",
                "1",
                "--opt-mode=optN",
