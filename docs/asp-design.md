...
| `opt_prev_seat_differ/0` | S-05 前回同席ペナルティの有効化 |
...
> 弱制約（S-05〜S-07）として実装されている。詳細は §3.3 を参照。
...
| S-03 | @4 | 班内男女バランス: 班に男女どちらかが0人（空班は対象外） | `opt_gender_balance` |
| S-04 | @4 | 孤独感緩和: 横隣∪同班に同性が1人もいない | `opt_loneliness` |
| S-05 | @1 | 前回同席ペナルティ | `opt_prev_seat_differ` |
...
| `--opt-mode=optN` | 最適スコアを求めた後に列挙（中間解も `on_model` に届く）。弱制約が無いケースでも `-n 1` で1モデル終了できる唯一の組み合わせ |
...
- **S-05**（`opt_prev_seat_differ`）: 全員が前回と異なる席になるよう最適化される
...
docker exec optiseat-backend-1 bash -c "
  clingo backend/solver/lp/base.lp backend/solver/lp/hard.lp backend/solver/lp/soft.lp \
         --opt-mode=optN --heuristic=Domain -n 1
"
